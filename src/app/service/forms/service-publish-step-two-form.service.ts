    ...
    public createForm(data?: StepTwoDataInterface): void {
        this.form = this.formBuilder.group({
                ...
                [ServicePublishStepTwoFormFields.FixedPrice]: [data?.price,
                    [
                        // data?.is_price_fixed ? Validators.required : Validators.nullValidator,
                        (this.form?.get(ServicePublishStepTwoFormFields.IsPriceFixed)?.value !== undefined &&
                            this.form?.get(ServicePublishStepTwoFormFields.IsPriceFixed)?.value) ?
                            Validators.required :
                            Validators.nullValidator
                    ]
                ],
                [ServicePublishStepTwoFormFields.StartPrice]: [
                    data?.start_price,
                    [
                        // (!data?.is_price_fixed && data?.is_price_fixed !== undefined) ? Validators.required : Validators.nullValidator,
                        (this.form?.get(ServicePublishStepTwoFormFields.IsPriceFixed)?.value !== undefined &&
                            !this.form?.get(ServicePublishStepTwoFormFields.IsPriceFixed)?.value) ?
                            Validators.required :
                            Validators.nullValidator
                ...
                [ServicePublishStepTwoFormFields.EndPrice]: [
                    data?.end_price,
                    [
                        // (!data?.is_price_fixed && data?.is_price_fixed !== undefined) ? Validators.required : Validators.nullValidator,
                        (this.form?.get(ServicePublishStepTwoFormFields.IsPriceFixed)?.value !== undefined &&
