...
export class MessagesListApiService extends AbstractReadonlyApiService<Message> {
    ...
    public getByInterlocutor(interlocutorId: number, pageSize?: number): Observable<ApiListResponseInterface<Message>> {
        return super.get({interlocutor: interlocutorId.toString(10), page_size: pageSize?.toString(10)});
    }

    public getUnreadCount(interlocutorId: number): Observable<number> {
        return super.get({is_read: 'false', interlocutor: interlocutorId.toString(10), sender: interlocutorId.toString(10)}).pipe(
            map(list => list.count)
        );
    }
