...
export class MessagesListApiService extends AbstractReadonlyApiService<Message> {
  ...
  public getByInterlocutor(
    interlocutorId: number,
    pageSize?: number,
    page?: number,
  ): Observable<ApiListResponseInterface<Message>> {
    return super.get({
      interlocutor: interlocutorId.toString(10),
  ...
  public getUnreadCount(interlocutorId: number): Observable<number> {
    return super.get({ is_read: 'false', sender: interlocutorId.toString(10) }).pipe(map(list => list.count));
  }
