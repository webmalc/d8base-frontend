...
export class CurrentPositionService {
                ...
                .then(() =>
                  this.geolocation
                    ...
                    .then((geo: Geoposition) =>
                      resolve({ latitude: geo.coords.latitude, longitude: geo.coords.longitude }),
                    ),
                )
                .catch(() => resolve(null))
            : this.geolocation
                .getCurrentPosition()
                .then((geo: Geoposition) => resolve({ latitude: geo.coords.latitude, longitude: geo.coords.longitude }))
                .catch(() => resolve(null)),
        )
        .catch(() => resolve(null)),
    );
  ...
  public getMergedLocationData(): Promise<UserLocation | null> {
    return new Promise<UserLocation | null>(resolve => {
      this.getCurrentPosition()
        .then((coords: Coords) => {
          this.getIpLocationData().subscribe((ipLocation: IpLocation) => {
  ...
  public getIpLocationData(): Observable<IpLocation | null> {
    return onErrorResumeNext(...this.ipServicesHolder.getList().map(service => service.getData())).pipe(
      first(),
